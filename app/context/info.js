...
const DataProvider = ({ children }) => {
  ...
  const cartIds =
    !cartLoading && cartData?.data
      ? cartData?.data.products?.map((x) => x.productId)
      : [];
  ...
  const { data: campaignData, isLoading: campaignLoading } = useSWR(
    !isOffline() ? "/store/campaign" : null
  );

  const campaign =
    !campaignLoading && campaignData?.data ? campaignData?.data : [];
  ...
  return (
    <DataContext.Provider
      value={{
        ...
        selectedAddress,
        cart,
        cartData: cartData?.data ? cartData?.data : {},
        pendingReviews:
