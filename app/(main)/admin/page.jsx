    ...
    <StoreWrapper>
      <Box className="px-1 md:px-6">
        <Grid container spacing={2}>
              ...
              <Box className="p-3 flex items-center justify-evenly">
                <Box className="w-9/12">
                  <Box>
                    ...
                    >
                      Good {getTimeOfDay()}, {adminInfo.name}
                    </Typography>
                  ...
                  <Box className="flex items-start justify-beteen !mt-5">
                    <DigitWithTopTag
                      tag="Today’s Visit"
                      digit={
                        (analytics && analytics?.todaysVisit[0]?.count) || 0
                      }
                    />
          ...
          <Grid item xs={12} sm={6}>
            <Box className="bg-white rounded-xl px-6 py-2 pt-2 w-full">
              <OptionsMenu
                icon={
              ...
              <Grid container className="bg-white">
                <Grid item xs={6} md={4}>
                  <DigitWithTopTag
                    tag="Orders"
                    digit={stat?.orders?.thisPeriodSum?.toLocaleString()}
                    percentage={stat?.orders?.percentage}
                    className="border-r my-1.5 py-2 items-center"
                  />
                </Grid>
                <Grid item xs={6} md={4}>
                  <DigitWithTopTag
                    tag="Items sold"
                    digit={stat?.itemsSold?.thisPeriodSum?.toLocaleString()}
                    percentage={stat?.itemsSold?.percentage}
                    className="md:border-r my-1.5 py-2 items-center"
                  />
                </Grid>
                <Grid item xs={6} md={4}>
                  <DigitWithTopTag
                    tag="Refund"
                    digit={formatCurrency(0)}
                    className=" border-r md:border-0 my-1.5 py-2 items-center"
                  />
                </Grid>
                <Grid item xs={6} md={4}>
                  <DigitWithTopTag
                    tag="Total revenue"
                    percentage={stat?.sales?.percentage}
                    digit={formatCurrency(stat?.sales?.thisPeriodSum)}
                    className="md:border-r my-1.5 py-2 items-center"
                  />
                </Grid>
                <Grid item xs={6} md={4}>
                  <DigitWithTopTag
                    tag="Total Inventory"
                    digit={stat?.inventory?.thisPeriodSum?.toLocaleString()}
                    percentage={stat?.inventory?.percentage}
                    className="border-r my-1.5 py-2 items-center"
                  />
                </Grid>
                <Grid item xs={6} md={4}>
                  <DigitWithTopTag
                    ...
                    digit={"4/5"}
                    percentage={53}
                    className="my-1.5 py-2 items-center"
                  />
                </Grid>
              </Grid>
            </Box>
